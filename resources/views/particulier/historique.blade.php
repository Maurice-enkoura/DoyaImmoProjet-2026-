...
<div class="view active">
                    ...
                    <div class="timeline-content">
                        <div class="event-header">
                            <div class="event-title">
                                <span class="event-icon" style="background:{{ $event['color'] }};">
                                    <i class="{{ $event['icon'] }}" style="color:#fff;font-size:14px;"></i>
                                </span>
                                <span class="event-name">{{ $event['title'] }}</span>
                            </div>
                            <div class="event-badge">
                                @if(isset($event['status']))
                                    <span class="status-pill status-{{ $event['status_class'] ?? 'default' }}">
                                        {{ $event['status'] }}
                                    </span>
                                @endif
                            </div>
                        ...
                        <div class="event-body">
                            ...
                            @if(isset($event['medias']) && $event['medias']->count() > 0)
                                <div class="event-medias">
                                    @php
                                        $images = $event['medias']->where('type_media', 'image');
                                        $videos = $event['medias']->where('type_media', 'video');
                                    @endphp

                                    @if($images->count() > 0)
                                        <div class="media-grid">
                                            @foreach($images->take(4) as $media)
                                                <div class="media-item" onclick="openLightbox('{{ asset('storage/' . $media->fichier) }}')">
                                                    <img src="{{ asset('storage/' . $media->fichier) }}" alt="Média">
                                        ...
                                        <div class="media-videos">
                                            @foreach($videos->take(2) as $media)
                                                <div class="media-video">
                                                    <video src="{{ asset('storage/' . $media->fichier) }}" controls></video>
                                                </div>
                                            @endforeach
                                        </div>
                                    @endif
                                    ...
                                    <div class="media-count">
                                        <span><i class="fa-regular fa-image"></i> {{ $images->count() }} photo(s)</span>
                                        <span><i class="fa-regular fa-circle-play"></i> {{ $videos->count() }} vidéo(s)</span>
                                    </div>
                                </div>
                            @endif
    ...
    @else
        <div class="empty-state">
            ...
            <p>Commencez à publier des demandes pour voir votre activité apparaître ici.</p>
