...
<head>
    ...
    <title>Rendez-vous terminé — DoyaImmo</title>
</head>
    ...
    <div style="background-color: #f4f4f4; padding: 40px 20px;">
        <div style="max-width: 600px; margin: auto; background: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 16px rgba(0,0,0,0.1);">
            ...
            <div style="padding: 32px;">
                <h2 style="color: #1A1A2E; margin-top: 0; font-size: 22px;"> Rendez-vous terminé</h2>
                ...
                <p style="color: #4A4A6A; font-size: 16px; line-height: 1.6;">
                    Votre rendez-vous a été marqué comme <strong>terminé</strong>.
                </p>
                
                <div style="background-color: #F7F9FC; border: 1px solid #E8E8F0; border-radius: 8px; padding: 20px; margin: 24px 0;">
                    <h3 style="color: #1A1A2E; margin-top: 0; font-size: 18px;"> Détails du rendez-vous</h3>
                    
                    <table style="width: 100%; border-collapse: collapse; margin-top: 12px;">
                        <tr>
                            <td style="padding: 8px 0; color: #8A8AA0; font-size: 14px; width: 40%;"> Date :</td>
                            <td style="padding: 8px 0; color: #4A4A6A; font-size: 16px; font-weight: bold;">
                                {{ $rendezVous->date_visite ? $rendezVous->date_visite->format('d/m/Y') : 'N/A' }}
                            </td>
                        </tr>
                        <tr>
                            <td style="padding: 8px 0; color: #8A8AA0; font-size: 14px;"> Heure :</td>
                            <td style="padding: 8px 0; color: #4A4A6A; font-size: 16px; font-weight: bold;">
                                {{ $rendezVous->heure_visite ?? $rendezVous->creneau->heure_debut ?? 'N/A' }}
                            </td>
                        </tr>
                        <tr>
                            <td style="padding: 8px 0; color: #8A8AA0; font-size: 14px;"> Agence :</td>
                            <td style="padding: 8px 0; color: #4A4A6A; font-size: 16px; font-weight: bold;">
                                {{ $rendezVous->agence->nom_agence ?? 'N/A' }}
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <p style="color: #4A4A6A; font-size: 16px; line-height: 1.6;">
                    Merci d'avoir utilisé <strong>DoyaImmo</strong> pour votre recherche immobilière.
                </p>
                
                <p style="color: #4A4A6A; font-size: 16px; line-height: 1.6;">
                    N'oubliez pas de laisser un avis sur cette agence pour aider les autres utilisateurs !
                </p>
                ...
                <div style="background-color: #F7F9FC; border: 1px solid #E8E8F0; border-radius: 8px; padding: 16px; margin-top: 24px;">
                    <p style="margin: 0; color: #4A4A6A; font-size: 14px; line-height: 1.6;">
                        <strong> Astuce :</strong> Connectez-vous à votre espace DoyaImmo pour laisser un avis sur l'agence.
                    </p>
                </div>
                
                <div style="text-align: center; margin: 32px 0;">
                    <a href="{{ $rendezVous->agence ? url('/particulier/evaluations/create/' . $rendezVous->agence->id) : '#' }}" 
